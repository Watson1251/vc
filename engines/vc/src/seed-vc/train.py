...
class Trainer:
    def __init__(self,
        ...
        # Select CUDA device if available, otherwise raise error
        if device == "cpu" or not torch.cuda.is_available():
            raise RuntimeError("CUDA device is required for training, but not available.")
        self.device = device if device.startswith("cuda") else "cuda:0"
        print(f" ------------------ {self.device}")

        self.data_dir = data_dir
        self.run_name = run_name
        config = yaml.safe_load(open(config_path))
        self.log_dir = os.path.join(data_dir, config['log_dir'], run_name)
        ...
        # after
        self.base_lr = float(scheduler_params['base_lr'])
        self.warmup_steps = int(scheduler_params['warmup_steps'])
    ...
    def _set_lr_for_key(self, key: str, lr: float):
        """
        Be liberal about how the optimizer wrapper stores torch.optim.Optimizer.
        """
        opt = None
        if hasattr(self.optimizer, "get"):               # some wrappers expose .get("cfm")
            try: opt = self.optimizer.get(key)
            except Exception: opt = None
        if opt is None and hasattr(self.optimizer, "optimizers"):
            opt = self.optimizer.optimizers.get(key, None)
        if opt is None and hasattr(self.optimizer, key):
            opt = getattr(self.optimizer, key)
        if opt is None:
            return
    ...
    def _apply_warmup(self):
        if self.warmup_steps <= 0:
            return 1.0
        # self.iters is incremented in the loop; use it to scale LR
        factor = min(1.0, float(self.iters) / float(self.warmup_steps))
        scaled = max(1e-12, self.base_lr * factor)
        # keys used right below in your step calls
        for k in ("cfm", "length_regulator"):
            self._set_lr_for_key(k, scaled)
        return factor
    ...
    def train_one_step(self, batch):
        ...
        # NEW: linear warmup of LR for first warmup_steps iterations
        warmup_factor = self._apply_warmup()
        ...
        # Only advance your repo's scheduler after warmup to avoid double-scaling
    ...
    def train(self):
        ...
        os.makedirs(self.log_dir, exist_ok=True)
        save_dir = os.path.join(self.data_dir, self.run_name)
        os.makedirs(save_dir, exist_ok=True)
        save_path = os.path.join(save_dir, 'ft_model.pth')
        torch.save(state, save_path)
...
if __name__ == '__main__':
    ...
    trainer = Trainer(
        ...
        num_workers=args.num_workers,
        device=args.device,
        base_lr=args.base_lr,                  # NEW
        warmup_steps=args.warmup_steps,        # NEW
    )
    trainer.train()
