...
export const CourcesReducer = createReducer(
    ...
    on(addcourcelistDataSuccess, (state, { newData }) => {
        return { ...state, productlist: [newData, ...state.courcelistdata], error: null };
    ...
    on(addcourcegridDataSuccess, (state, { newData }) => {
        return { ...state, productgrid: [newData, ...state.Griddata], error: null };
    ...
    on(deletecourcegridSuccess, (state, { id }) => {
        const updatedgrid = state.Griddata.filter((Griddata) => !id.includes(Griddata.id));
