...
import { fetchproductsData } from 'src/app/store/Ecommerce/ecommerce.actions';
import { selectData } from 'src/app/store/Product/product.selector';
import { PageChangedEvent } from 'ngx-bootstrap/pagination';
import { addproductsData, deleteproductsData, updateproductsData } from 'src/app/store/Product/product.action';
...
export class ProductsComponent {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      ...
      this.store.select(selectData).subscribe((data) => {
        ...
        this.products = this.productlist.slice(0, 10);
      })
  ...
  */
  saveProduct() {
    if (this.productForm.valid) {
      if (this.productForm.get('id')?.value) {
        ...
        this.store.dispatch(updateproductsData({ updatedData }));
      }
      else {
        ...
        this.store.dispatch(addproductsData({ newData }));
      }
  ...
  deleteData(id: any) {
    ...
    if (id) {
      this.store.dispatch(deleteproductsData({ id: this.deleteId.toString() }));
    }
    this.store.dispatch(deleteproductsData({ id: this.checkedValGet.toString() }));
    this.deleteRecordModal?.hide();
  ...
  filterdata() {
    if (this.term) {
      this.products = this.productlist.filter((el: any) => el.title.toLowerCase().includes(this.term.toLowerCase()))
    } else {
      this.products = this.productlist
    }
    // this.productlist = this.products.slice(0, 10);
    // noResultElement
