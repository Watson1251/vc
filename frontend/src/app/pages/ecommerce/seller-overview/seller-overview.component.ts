  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      ...
      this.store.select(selectoverviewData).subscribe((data) => {
        ...
        this.seller = data;
        this.sellerlist = this.seller.slice(0, 8);
      });
...
*/
  ...
  saveOverview() {
    if (this.overviewForm.valid) {
      if (this.overviewForm.get('id')?.value) {
        const updatedData = this.overviewForm.value;
        ...
        const newData = this.overviewForm.value
  ...
  checkUncheckAll(ev: any) {
    this.sellerlist = this.sellerlist.map((x: { states: any }) => ({ ...x, states: ev.target.checked }));
  ...
  filterdata() {
    if (this.term) {
      this.sellerlist = this.seller.filter((el: any) => el.category.toLowerCase().includes(this.term.toLowerCase()))
    } else {
